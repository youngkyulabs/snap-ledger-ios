...
SnapLedger/                          # 메인 앱 타겟 (synchronized root group)
  ...
  Features/                          # UI 화면
    ...
    Settings/                        # SettingsView (저장폴더+동기화 상태배지 / reminder / FM 상태),
...
7. **파일 동기화는 월 단위 통째 교체** (`SyncCoordinator`)
   ...
   - 모든 CSV 쓰기/import 후 지문을 갱신해 다음 감지의 기준으로 삼는다. 수동 동기화 진입점은 **설정 → 저장 폴더 섹션**: 맞춰져 있으면 "동기화됨" 배지, 아니면 "동기화" 버튼(`SyncCoordinator.folderSyncSummary`)이 `FileSyncView`로 연결. 그 화면에서 월별 상태(최신/파일 변경됨/파일에만 있음/앱에만 있음)를 보고 월 단위 또는 전체로 가져오기/저장 (`SyncCoordinator.monthStatuses`).
