...
찰칵가계부 App Store Connect 제출용 메타데이터 (현재 버전 1.3).
...
### What's New in This Version (1.3 출시 노트)
```
• 최근 기록 검색 — 가맹점·카테고리·메모는 물론 금액으로도 지난 지출을 바로 찾을 수 있어요.
• 검토 날짜 확인 — 캡처한 날짜가 오래됐거나 미래로 잡히면 검토 화면에서 경고 아이콘으로 짚어줘, 저장 전에 한 번 더 확인할 수 있어요.
• 자동 추출 정확도 개선 — 날짜를 지어내거나, 결제 한 건을 두 건으로 잘못 나누거나, 예전에 지운 카테고리로 잘못 채우는 경우를 줄였어요.
```
