...
• 스크린샷을 찍지 않아도 문자·메신저의 결제 알림 문구를 선택해 공유하면 그대로 추출
...
• 최근 기록 탭에서 날짜별로 묶어 보기, 스크롤하면 과거 달이 자동으로 이어짐.
  가맹점·카테고리·메모·금액으로 검색, 다중 선택 후 Numbers·메일로 바로 붙여넣기
• 정산하기 → 전체 진행률 → 카테고리 비중 도넛 → 월별 추세까지 한 화면에 이어지는 월간 요약 탭
...
• 월간 요약 탭 — 흩어져 있던 예산과 통계를 한 탭으로 합쳤어요. 월 정산부터 전체 진행률, 카테고리 비중, 월별 추세까지 한 스크롤로 이어지고, 진행률이나 도넛을 누르면 카테고리별로 더 파고들 수 있어요. 지난달을 고르면 추세 그래프도 그 달에 맞춰 움직입니다.
