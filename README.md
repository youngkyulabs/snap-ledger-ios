...
1. iOS 공유 시트에서 찰칵가계부 선택 — 이미지뿐 아니라 문자·메신저의 결제 알림 **텍스트**도 그대로 공유 가능 (또는 Shortcuts/Siri로 "이미지에서 지출 추가")
2. 이미지 또는 공유한 텍스트가 App Group inbox에 큐잉되고 찰칵가계부 앱이 백그라운드에서 처리
3. **VisionKit OCR**로 한·영 텍스트 추출 (이미지만 — 공유된 텍스트는 OCR을 건너뜀)
...
- **텍스트 공유 수신** — 스크린샷 없이 문자·메신저의 결제 알림 문구를 선택해 공유해도 동일 파이프라인으로 추출 (`.txt`로 inbox에 저장, 추출 직전 2,000자 클램프)
...
- **최근 기록 탭** — 저장한 지출을 일별로 보기 + 가맹점·카테고리·메모·금액 검색 + 내보낸 월별 CSV 뷰어와 다중 선택 복사 (HTML+TSV 멀티 페이로드로 Numbers에 표 그대로 paste)
...
- **월간 요약 탭** — 예산과 통계를 한 탭으로 통합 (탭 4개: 검토·최근 기록·월간 요약·설정). 월 정산 → 전체 진행률 → 카테고리 비중 도넛 → 월별 추세가 한 스크롤로 이어지고, 진행률·도넛을 누르면 카테고리별 드릴다운 시트로 진입
...
- **월 정산** — 수입·카드 사용액(이번 달 + 이번 달 빠져나간 전월 카드대금)·저축·계좌별 잔액·자금 변동을 입력해 '실제 쓴 돈'과 '기록한 돈'을 대조. 전월 값은 자동 이월 프리필되고, 이미 정산을 마친 달에 항목을 추가하면 저장 전 경고. 결과는 저장 폴더에 `reconciliations-YYYY-MM.csv`로 내보내져 AI에게 그대로 분석을 맡길 수 있음 (지출 CSV와 동일한 한 방향 export)
