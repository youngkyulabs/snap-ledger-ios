...
- **App Store metadata lives in `fastlane/metadata/ko/`**: All fields are tracked, not just the ones being changed — deliver silently skips fields whose files are absent, which is how Promotional Text went missing in 1.4. `fastlane/screenshots/` is generated from `internal/screenshot/` by `scripts/stage_screenshots.sh` and is gitignored. Run `python3 -m scripts.asc_release audit` any time to compare the repo against App Store Connect. It needs three environment variables — `ASC_KEY_PATH` (the App Store Connect `.p8`, kept outside the repo under `~/.appstoreconnect/private_keys/`), `ASC_KEY_ID` and `ASC_ISSUER_ID`; in CI these come from the `ASC_PRIVATE_KEY`, `ASC_KEY_ID` and `ASC_ISSUER_ID` repository secrets, alongside the `XCODE_CLOUD_TAG_WORKFLOW_ID` repository variable. `python3 -m scripts.asc_release preflight --tag vX.Y --workflow-id $ID` runs the same checks the release workflow gates on, without uploading anything.
