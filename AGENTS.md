...
- **Release is tag-triggered**: Pushing a `v*` tag starts two things in parallel. The Xcode Cloud `Tag Release` workflow archives and uploads to TestFlight — it deliberately has **no path filter**, because release tags sit on docs commits that the main-push filter would skip. It was created once through the App Store Connect API rather than the Xcode UI; if it ever needs recreating, the request body and the reasoning behind it are in this branch's history (`scripts/create_tag_workflow.py`, removed after it had done its one job). The GitHub Actions `Release` workflow (`.github/workflows/release.yml`) uploads metadata and screenshots via `fastlane deliver`, then waits for that build to reach `VALID` and links it to the App Store version. **Review submission stays manual.** The job refuses any tag that is not an ancestor of `main`, dereferences the tag to its commit (`^{commit}`) so annotated tags still match Xcode Cloud's `sourceCommit`, and keeps `timeout-minutes` above `link-build`'s own wait budget so a slow build ends in the script's error rather than a bare job cancellation.
- **App Store metadata lives in `fastlane/metadata/ko/`**: All seven localized fields deliver uploads (subtitle, promotional text, keywords, description, release notes, support URL, privacy URL) are tracked, not just the ones being changed — deliver silently skips fields whose files are absent, which is how Promotional Text went missing in 1.4, so `preflight` blocks on a missing file rather than warning. It also blocks when `release_notes.txt` is still byte-identical to the What's New already published for the previous version — presence and length checks both pass on a file nobody rewrote, and `audit` compares it against the very text it was copied from and reports a match, so nothing else would notice the release shipping the last version's changelog. App-level fields deliver *could* manage but that are deliberately **not** tracked here (copyright, categories, age rating, review information) are set directly in App Store Connect and recorded in `internal/app-store-submission.md`, which holds the rationale for the metadata values but never a second copy of them. `fastlane/screenshots/` is generated from `internal/screenshot/` by `scripts/stage_screenshots.sh` and is gitignored. Run `python3 -m scripts.asc_release audit` any time to compare the repo against App Store Connect. It needs three environment variables — `ASC_KEY_PATH` (the App Store Connect `.p8`, kept outside the repo under `~/.appstoreconnect/private_keys/`), `ASC_KEY_ID` and `ASC_ISSUER_ID`; in CI these come from the `ASC_PRIVATE_KEY`, `ASC_KEY_ID` and `ASC_ISSUER_ID` repository secrets, alongside the `XCODE_CLOUD_TAG_WORKFLOW_ID` repository variable. `python3 -m scripts.asc_release preflight --tag vX.Y --workflow-id $ID` runs the same checks the release workflow gates on, without uploading anything.
...
9. **Monthly Reconciliation Splits Header + Item Models, Round-Trips to CSV (`MonthlyReconciliation` + 5 item models / `ReconciliationStore` / `ReconciliationCSV`)**
   ...
   - **Summary and Verdict Centralized in `ReconciliationSummary` (pure)**: Calculates `actualSpending`, `recordedSpending`, difference, and `isReconciled(status:)` in one place so the Budget tab and reconciliation view reach identical conclusions. An in-progress month is considered "in progress" only after the user enters actual numbers (closing balance != opening balance, or this month's card usage); prior to that, `actualSpending` is gated to 0 to prevent prefill noise. `previousAmount` is deliberately excluded from that test because carry-forward prefills it. Closed past months are finalized if saved data exists.
