...
- **App Store metadata lives in `fastlane/metadata/ko/`**: All fields are tracked, not just the ones being changed — deliver silently skips fields whose files are absent, which is how Promotional Text went missing in 1.4. `fastlane/screenshots/` is generated from `internal/screenshot/` by `scripts/stage_screenshots.sh` and is gitignored. Run `python3 -m scripts.asc_release audit` any time to compare the repo against App Store Connect.
